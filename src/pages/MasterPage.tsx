...
function WorkCard({ index, go }: { index:number; go:(screen:Screen)=>void }) {
  return <article className="srez-work-card"><button className="srez-work-card__visual" onClick={() => go('Work')}><Media index={index} ratioOverride={.82} /></button><div className="srez-work-card__text"><strong>Mullet</strong><span>Есть опубликованная работа</span></div></article>;
}
...
export function MasterPage({ state, go, onSave }: Props) {
  ...
  return <div className="srez-app"><Header screen={state.screen} saved={state.saved} go={go} /><main className="srez-shell">
    <section className="srez-detail-head"><button className="srez-back" onClick={() => go('Catalog')}>← Назад к результатам</button><div className="srez-master-hero"><div className="srez-master-hero__visual"><Media index={0} ratioOverride={.72} /></div><div className="srez-master-hero__info"><span className="srez-eyebrow">МАСТЕР</span><h1>{master.name}</h1><div className="srez-master-hero__place">{master.place}</div><Evidence master={master} /><div className="srez-master-hero__actions"><Button>Перейти к записи</Button><button className={`cosmos-save ${state.saved?'is-saved':''}`} onClick={onSave}>♡ <span>{state.saved?'Saved':'Save'}</span></button></div></div></div></section>
    <section className="srez-detail-section"><span className="srez-eyebrow">EVIDENCE FIRST</span><div className="cosmos-section-heading"><h2>Работы и основания по Mullet</h2><button>Все работы</button></div><div className="srez-work-grid">{[0,1,2,3].map(i => <WorkCard index={i} go={go} key={i} />)}</div><div className="srez-proto-note srez-proto-note--section">Визуалы — плейсхолдеры прототипа. Реальные источники отделены от иллюстраций.</div></section>
