...
export function FavouritesPage({ state, go, onSave }: Props) {
  return <div className="srez-app"><Header screen={state.screen} saved={state.saved} go={go} selectedLook={state.focus ? 'Mullet' : null} /><main className="srez-shell"><section className="srez-favs">{state.saved ? <div className="srez-master-grid"><MasterCard master={MASTERS[0]} saved={state.saved} onSave={onSave} go={go} /></div> : <div className="srez-empty"><div className="srez-empty__mark">∅</div><h3>В избранном пока пусто</h3><p>Добавь мастера сердечком в выдаче или профиле.</p><button type="button" className="cosmos-button cosmos-button--secondary cosmos-button--sm" onClick={() => go('Catalog')}>Посмотреть мастеров</button></div>}</section></main></div>;
}
